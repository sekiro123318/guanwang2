...
function ProgramCard({
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-6 transition-shadow hover:shadow-lg">
      ...
      {/* Progress bar - Adjusted to max at IND-Enabling stage */}
      <div className="mt-6">
        <div className="flex items-center justify-between text-xs text-muted-foreground mb-1.5">
          <span>{t("pipeline.progressToInd")}</span>
          <span className="font-semibold text-foreground">
            {program.activeStage >= 3 ? "IND前" : `${program.progress}%`}
          </span>
        </div>
        <div className="h-2 overflow-hidden rounded-full bg-secondary">
          <div
            ...
            style={{ 
              width: program.activeStage >= 3 ? "100%" : `${program.progress}%` 
            }}
          />
        </div>
      </div>
